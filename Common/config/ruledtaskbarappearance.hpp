...
struct RuledTaskbarAppearance : OptionalTaskbarAppearance
{
    ...
    RuledTaskbarAppearance(std::unordered_map<std::wstring, ActiveInactiveTaskbarAppearance> classRules,
                           std::unordered_map<std::wstring, ActiveInactiveTaskbarAppearance> titleRules,
                           win32::FilenameMap<ActiveInactiveTaskbarAppearance> fileRules, bool enabled,
...
private:
    ...
    inline static void SerializeRulesMap(Writer& writer,
                                         const std::unordered_map<
                                             std::wstring, ActiveInactiveTaskbarAppearance, Hash, Equal, Alloc>& map,
                                         std::wstring_view mapKey)
    {
    ...
    inline static void DeserializeMap(const rjh::value_t& obj,
                                      std::unordered_map<
                                          std::wstring, ActiveInactiveTaskbarAppearance, Hash, Equal, Alloc>& map,
                                      void (*unknownKeyCallback)(std::wstring_view))
    {
        rjh::EnsureType(rj::Type::kObjectType, obj.GetType(), L"root node");
