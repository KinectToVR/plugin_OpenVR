...
namespace rjh
{
    ...
    {
        if (!IsType(expected, actual))
        [[unlikely]]
        {
            throw DeserializationError{
                std::format(L"Expected {} but found {} while deserializing {}", TYPE_NAMES.at(expected),
                            TYPE_NAMES.at(actual), obj)
            };
        }
    }
    ...
    inline void Serialize(Writer& writer, const T& member, std::wstring_view key,
                          const std::array<std::wstring_view, size>& arr)
    {
    ...
    template <typename T, std::size_t size>
        requires std::is_enum_v<T>
    inline void Deserialize(const value_t& obj, T& member, std::wstring_view key,
                            const std::array<std::wstring_view, size>& arr)
    {
