...
namespace amethyst::driver::implementation
{
    ...
    bool BodyTracker::set_pose(const TrackerBase::Reader& tracker)
    {
        try
        {
            // Position
            _pose.vecPosition[0] = tracker.getPosition().getX();
            _pose.vecPosition[1] = tracker.getPosition().getY();
            _pose.vecPosition[2] = tracker.getPosition().getZ();
            _valid = tracker.getTrackingState();

            // Rotation
            _pose.qRotation.w = tracker.getOrientation().getW();
            _pose.qRotation.x = tracker.getOrientation().getX();
            _pose.qRotation.y = tracker.getOrientation().getY();
            _pose.qRotation.z = tracker.getOrientation().getZ();

            // If the sender defines its own velocity
            if (tracker.getVelocity().getHasValue())
            {
                // Velocity
                _pose.vecVelocity[0] = tracker.getVelocity().getValue().getX();
                _pose.vecVelocity[1] = tracker.getVelocity().getValue().getY();
                _pose.vecVelocity[2] = tracker.getVelocity().getValue().getZ();
            }
            else
            {
                // Velocity
            ...
            if (tracker.getAcceleration().getHasValue())
            {
                // Acceleration
                _pose.vecAcceleration[0] = tracker.getAcceleration().getValue().getX();
                _pose.vecAcceleration[1] = tracker.getAcceleration().getValue().getY();
                _pose.vecAcceleration[2] = tracker.getAcceleration().getValue().getZ();
            }
            else
            {
                // Acceleration
            ...
            if (tracker.getAngularVelocity().getHasValue())
            {
                // Angular Velocity
                _pose.vecAngularVelocity[0] = tracker.getAngularVelocity().getValue().getX();
                _pose.vecAngularVelocity[1] = tracker.getAngularVelocity().getValue().getY();
                _pose.vecAngularVelocity[2] = tracker.getAngularVelocity().getValue().getZ();
            }
            else
            {
                // Angular Velocity
            ...
            if (tracker.getAngularAcceleration().getHasValue())
            {
                // Angular Acceleration
                _pose.vecAngularAcceleration[0] = tracker.getAngularAcceleration().getValue().getX();
                _pose.vecAngularAcceleration[1] = tracker.getAngularAcceleration().getValue().getY();
                _pose.vecAngularAcceleration[2] = tracker.getAngularAcceleration().getValue().getZ();
            }
            else
            {
                // Angular Acceleration
    ...
    bool BodyTracker::spawn()
    {
        try
        {
            if (!_added && !_serial.empty())
            {
                ...
                vr::VRServerDriverHost()->TrackedDeviceAdded(_serial.c_str(), vr::TrackedDeviceClass_GenericTracker,
                                                             this);
                _added = true;
                return true;
            }
    ...
    vr::EVRInitError BodyTracker::Activate(vr::TrackedDeviceIndex_t index)
    {
        ...
        if (is_hand())
        {
            ...
            vr::VRProperties()->SetStringProperty(_props, vr::Prop_NamedIconPathDeviceSearchingAlert_String,
                                                  std::format(
                                                      "{{oculus}}/icons/rifts_{}_controller_searching_alert.gif",
                                                      _type == Tracker_LeftHand ? "left" : "right").c_str());
        ...
        {
            vr::VRProperties()->SetInt32Property(_props, vr::Prop_DeviceClass_Int32,
