...
extern "C" __declspec(dllexport) void* HmdDriverFactory(const char* pInterfaceName, int* pReturnCode)
{
	...
	if (google::IsGoogleLoggingInitialized())
	{
		...
		ktvr::WStringToString(ktvr::GetK2AppDataLogFileDir(L"VRDriver", L"Amethyst_VRDriver_")) << "*.log\"";
	...
	// Init logging
	google::InitGoogleLogging(ktvr::WStringToString(thisLogDestination).c_str());

	// Delete logs older than 7 days
	google::EnableLogCleaner(7);

	// Log everything >=INFO to same file
	google::SetLogDestination(google::GLOG_INFO, ktvr::WStringToString(thisLogDestination).c_str());
	...
	if (IsUserAdmin())
	{
		...
		LOG(INFO) << "Got crash handler assumed location: " << ktvr::WStringToString(CHandlerPath.wstring());
