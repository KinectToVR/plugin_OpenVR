...
BodyTracker::BodyTracker(std::string serial, ITrackerType role)
{
    _serial = serial;
...
void BodyTracker::set_pose(void* tracker_ptr)
{
    try
    {
        // Generate a handle to the managed tracker object
        const gcroot<TrackerBase^>& tracker = *static_cast<gcroot<TrackerBase^>*>(tracker_ptr);
